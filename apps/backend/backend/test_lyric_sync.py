...
from backend.utils.lyric_synchronizer import LyricSynchronizer, SyncedLyricSegment, SyncedLyricsSegmentWithWordLevelTimestamp, SyncedLyricsSegmentWithWordLevelTimestampArgs
from os import path
import asyncio
...
async def run():
    ...
    with open(path.join(ElmiConfig.DIR_DATA, "test_lyric_segment_word_timestamps.json"), "r") as f:
        ...
        #print(data)
        for seg in data:
            await synchronizer.align_lyric_line_with_word_timestamps(SyncedLyricSegment(**seg.model_dump()), seg.words)
